...
public class Sng_StartNewProject : SingletonService
{
	...
	{
		AddOrActivateWebPageTab("Sample",
		...
		await SpeakAsync("Create a new page in Notion, and start by writing /linked to link the MainDB");
		await PauseAsync("Ready?");
		await SpeakAsync("Create a filter for the tag that you will create in the same process");
		await PauseAsync("Ready?");
		...
		await SpeakAsync("Now the result of the filter is empty. Press the plus sign button to add pages to that view");
		await PauseAsync("Ready?");
